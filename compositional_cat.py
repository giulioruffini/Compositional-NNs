...
def forward_kinematics(
    ...
    """
    Forward kinematics for a planar kinematic chain.
    Returns list of joint positions (including base).
    This is the Product of Exponentials in SO(2).
    """
...
class JointedCat:
    """
    A 2D articulated cat with compositional Lie-group hierarchy.

    The cat is built from kinematic chains:
    ...
    def render(self, img_size: int = 128) -> Image.Image:
        """
        Render the cat to a PIL image.
        Applies camera transform (Level 1) and appearance (Level 6)
...
# Parameter ranges for each level
LEVEL_PARAMS = {
    1: {  # Camera SE(2) x R+
        'cam_angle':  (-0.3, 0.3),
        'cam_tx':     (-0.4, 0.4),
...
# Named conditions: which levels are active
CONDITIONS = {
    'Static':           [],
    'CameraOnly':       [1],
    'CameraBody':       [1, 2],
...
def sample_params(
    ...
    """Sample parameters: active levels get random values, inactive get defaults."""
    cat = JointedCat()
...
def generate_dataset(
    ...
    """
    Generate a dataset for a given condition.

    Saves:
      - {output_dir}/{condition}/images/  — PNG images
      - {output_dir}/{condition}/metadata.jsonl — one JSON line per sample
    """
